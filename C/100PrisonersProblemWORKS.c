...
void initializeRoom(int room[], int roomSize) {
    for(int i=0; i<roomSize; i++) room[i] = i;      // initialize with elements 0-99 in order
...
bool findNumber(int prisonerNumber, int room[], int roomSize) {
    ...
    int boxToCheck = prisonerNumber;                        // first box to check (index) is the prisoner's number
    ...
    while(attemptCount <= maxAttempts) {                    // while attempts are less than max attempts,
        ...
        boxToCheck = room[boxToCheck];                          // otherwise, set box to check to contents of the box just checked
...
int testRoom(int room[], int roomSize) {
    for(int i=0; i<roomSize; i++){                  // for each prisoner
